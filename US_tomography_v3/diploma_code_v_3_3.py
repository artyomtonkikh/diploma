...
def lm_method(scaling_ini, sensor_count, radius):
    ...
    for iterate in range(maxiteration):
        ...
        diag_data=np.einsum('ij,ji->i',J.T.toarray(),J.toarray()) #ускорение подсчета диагонали через суммы Эйнштейна
        ...
        total_operator=aslinearoperator(J.T)@aslinearoperator(J)+aslinearoperator(matrix_from_diag)
...
n=100                                             #сетка области
sensor_count=16                                  #количество учитываемых сенсоров
